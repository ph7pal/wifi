...
class download {
    ...
    function setnewfn($filename) {
        $this->newfn = $filename;
    }

    function downloadfile($filename, $newfn) {
        $this->setfilename($filename);
        $this->setnewfn($newfn);
        if ($this->filecheck()) {
            $fn = array_pop(explode('/', strtr($this->filename, '\\', '/'))); // file name change
            $extend_name = preg_replace("/.*\./si", "", $fn);
            $fn = $this->newfn . '.' . $extend_name;
